...
class PartnerLedgerReport(models.AbstractModel):
    ...
    def render_html(self,docids, data=None):
        ...
        def opening(bal):
            if typed == "all":
                opend = self.env['account.move.line'].search([('move_id.date','>',form),('move_id.date','<',to),('account_id.id','=',bal.id)])

            if typed == "posted":
                opend = self.env['account.move.line'].search([('move_id.date','>',form),('move_id.date','<',to),('account_id.id','=',bal.id),('move_id.state','=',"posted")])
